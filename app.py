...
if not st.session_state.setup_complete:
    ...
    with col2:
        st.session_state.position = st.selectbox(
            "Choose a position",
            options=["Software Engineer", "Data Scientist", "Product Manager", "UX Designer", "Other"]
        )

    st.session_state.company = st.selectbox(
        "Choose a company",
        ("Amazon", "Apple", "Google", "Microsoft", "Other")
    )
...
if st.session_state.setup_complete and not st.session_state.feedback_shown and not st.session_state.chat_complete:
    ...
    if not st.session_state.messages:
        st.session_state.messages = [{"role": "system", "content": f"""You are an HR HR executive that interviews an interviewee called {st.session_state.name}
                                      with experience in {st.session_state.experience} and skills in {st.session_state.skills}.
                                      You should interview him for the position {st.session_state.level} {st.session_state.position} at the company {st.session_state.company}.
                                      """}]
